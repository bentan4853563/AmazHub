...
export function getAllPortfolios(category?: string): Portfolio[] {
  ...
  const portfolios = slugs
    ...
    .filter((portfolio) => (category && category !== 'All') ? portfolio.categories.includes(category) : true) // Filter by category
